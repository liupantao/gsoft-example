...
layui.define(['laytpl', 'laypage', 'layer', 'form'], function(exports){
          ...
          ,'<th data-field="{{ item2.field||i2 }}" {{# if(item2.minWidth){ }}data-minwidth="{{item2.minWidth}}"{{# } }} '+ rowCols +' {{# if(item2.unresize){ }}data-unresize="true"{{# } }}>'
            ,'<div class="layui-table-cell laytable-cell-'
              ,'{{# if(item2.colspan > 1){ }}'
                ,'group'
              ,'{{# } else { }}'
                ,'{{d.index}}-{{layer.getStyleField(item2.field) || i2}}'
  ...
  ,TPL_MAIN = ['<div class="layui-form layui-border-box {{d.VIEW_CLASS}}" lay-filter="LAY-table-{{d.index}}" style="{{# if(d.data.width){ }}width:{{d.data.width}}px;{{# } }} {{# if(d.data.height){ }}height:{{d.data.height}}px;{{# } }}">'
    ...
    ,'{{# layui.each(d.data.cols, function(i1, item1){'
      ,'layui.each(item1, function(i2, item2){ }}'
        ,'.laytable-cell-{{d.index}}-{{layer.getStyleField(item2.field)||i2}}{ '
        ,'{{# if(item2.width){ }}'
    ...
    ,render = function(){ //后续性能提升的重点
      ...
      layui.each(data, function(i1, item1){
        ...
        that.eachCols(function(i3, item3){
          var field = item3.field || i3, content = layer.getPropertyValue(item1,field)
          ...
          var td = ['<td data-field="'+ field +'" '+ function(){
            ...
            ,'<div class="layui-table-cell laytable-cell-'+ function(){ //返回对应的CSS类标识
              var str = (options.index + '-' + layer.getStyleField(field));
  ...
  Class.prototype.getColElem = function(parent, field){
    var that = this
    ,options = that.config;
    return parent.eq(0).find('.laytable-cell-'+ (options.index + '-' + layer.getStyleField(field)) + ':eq(0)');
  };
  ...
  Class.prototype.sort = function(th, type, pull, formEvent){
    ...
    try {
      ...
      var elemSort = that.layHeader.find('th .laytable-cell-'+ options.index +'-'+ layer.getStyleField(field)).find(ELEM_SORT);
  ...
  Class.prototype.getCssRule = function(field, callback){
    ...
    layui.each(rules, function(i, item){
      if(item.selectorText === ('.laytable-cell-'+ that.index +'-'+ layer.getStyleField(field))){
  ...
  table.reload = function(id, options){
    ...
    if(options.where)delete config.where;
