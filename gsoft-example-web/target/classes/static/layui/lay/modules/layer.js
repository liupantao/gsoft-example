...
var layer = {
  ...
  ajax: function(ajaxOptions,isTimeStamp){
		...
		if(ajaxOptions.notShowLoading!=true){
			load_layer = layer.msg(ajaxOptions.wait||'努力中...', {icon: 16,shade: [0.5, '#f5f5f5'],scrollbar: false,offset: '0px', time:100000});
		}
  ...
  getConvert: function(converts){
	  ...
	  if(converts){
		...
		layer.ajax({
			url: "/dubbo/local/convertManager/getConverts.json",
			data:params.join('&'),
			notShowLoading:true,
			async:false,
  ...
  getStyleField: function(field){
	  if(!field)return field;
	  var fieldS;
	  try {
		  var fieldS = field.replace(/\./g,'_');
	  } catch (e) {
		  fieldS = field;
	  }
	  return fieldS;
